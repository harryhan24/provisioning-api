...
export default class LookupService {
  static async byAccountName(accountName: string) {
    logger.debug(`[LookupService] Fetching user details by account name (${accountName})`, { tags: "lookupService, byAccountName" });
    ...
    const auEduPersonSharedToken = await IdpApi.findAafTokenByUsername(accountName);
  ...
  static async staffSearch(searchString: string) {
    logger.debug(`[LookupService] Search staff with string (${searchString})`, { tags: "lookupService, staffSearch" });
