...
describe("The LookupService->byAccountName", () => {
  test("should emit a debug log when it is called", () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith(`[LookupService] Fetching user details by account name (account_name)`, {
...
describe("The LookupService -> staffSearch", () => {
  test("should emit a debug log when it is called", () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith(`[LookupService] Search staff with string (account_name)`, {
