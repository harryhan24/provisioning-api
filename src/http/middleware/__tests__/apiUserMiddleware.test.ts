...
import apiUserMiddleware from "../apiUserMiddleware";
import { ApiUser } from "../../../database/models";

jest.unmock("../apiUserMiddleware");
...
describe("The ApiUser middleware", () => {
  ...
  test("should return an error if no authorization header is provided", done => {
    ...
    apiUserMiddleware({ originalUrl: "/", headers: {} }, mockResponse, callback).then(() => {
      expect(callback).toHaveBeenCalledTimes(0);
      expect(mockResponse.hasStatus).toBe(403);
      expect(mockResponse.hasBody.statusCode).toBe(403);
      expect(mockResponse.hasBody.message).toBe("No authorization headers provided");
      done();
    });
  });
  ...
  test("should return an error if no ApiKey is provided", done => {
    const callback = jest.fn(() => {});
    const mockResponse = new MockResponse();
    apiUserMiddleware({ originalUrl: "/", headers: { authorization: "" } }, mockResponse, callback).then(() => {
      expect(callback).toHaveBeenCalledTimes(0);
      expect(mockResponse.hasStatus).toBe(403);
      expect(mockResponse.hasBody.statusCode).toBe(403);
      expect(mockResponse.hasBody.message).toBe("No authorization headers provided");
  ...
  test("should return an error if an empty ApiKey is provided", done => {
    const callback = jest.fn(() => {});
    const mockResponse = new MockResponse();
    apiUserMiddleware({ originalUrl: "/", headers: { authorization: "ApiKey " } }, mockResponse, callback).then(() => {
      expect(callback).toHaveBeenCalledTimes(0);
      expect(mockResponse.hasStatus).toBe(403);
      expect(mockResponse.hasBody.statusCode).toBe(403);
  ...
  test("should return an error if the ApiUser Sequelize instance throws an error", done => {
    ...
    apiUserMiddleware({ originalUrl: "/", headers: { authorization: "ApiKey abc" } }, mockResponse, callback).then(() => {
      expect(callback).toHaveBeenCalledTimes(0);
      expect(mockResponse.hasStatus).toBe(400);
      expect(mockResponse.hasBody.statusCode).toBe(400);
