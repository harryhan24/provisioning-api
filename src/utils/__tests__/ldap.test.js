...
describe("The Ldap -> search function", () => {
  test("should handle errors", () => {
    const l = new Ldap("www.google.com", "baseDn", "user", "password");
    l.bind = jest.fn();
    libLdap.client.search.mockImplementationOnce(
      (baseDn, options, callback) => {
        callback(new Error("some_error"), null);
      },
    );

    expect(l.search("a")).rejects.toEqual(
      new Error("Search failed with message: some_error"),
    );
  });
  ...
  test("should handle a late error", () => {
    ...
    expect(l.search("a")).rejects.toEqual(
      new Error("Search error: some_error"),
    );
  });
...
describe("The Ldap -> searchFirst function", () => {
  test("should catch errors and return null", () => {
    ...
    expect(logger.log).toHaveBeenCalledWith(
      "error",
      "[Ldap] Search failed with error: some_error",
    );
  });
...
describe("The Ldap -> cleanLdapEntry function", () => {
  ...
  test("should handle arrays", () => {
    expect(Ldap.cleanLdapEntry([{ key: "true" }, { key: "false" }])).toEqual([
      { key: true },
      { key: false },
    ]);
  });
