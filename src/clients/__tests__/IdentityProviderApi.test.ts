...
describe("The IdentityProviderApi", () => {
  ...
  test("should be able to search for an AAF token and catch errors", done => {
    IdpApi.client.get.mockImplementationOnce(() => new Promise((resolve, reject) => reject(new Error("a"))));

    IdpApi.findAafTokenByUsername("some_user").then(result => {
      expect(result).toEqual(null);
      expect(logger.log).toHaveBeenCalledWith("error", `Could not get complete "auEduPersonSharedToken" lookup: a`);
      done();
    });
  });
